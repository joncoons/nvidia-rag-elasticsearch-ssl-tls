...
"""
crawl tool — streaming-batch BFS web crawler for the NVIDIA RAG ingestor.

This module is the canonical home for the web crawl pipeline.
nvidia_rag.utils.web_crawler is a compatibility shim that re-exports from here.
...
Phase 3 -- Binary ingest: NFS-persisted documents (PDFs, DOCX, etc.) that
    were downloaded during Phase 1 are batch-ingested using the
    nvidia_rag.tools.parse_document tool for complex PDFs and
    nvidia_rag.storage.embed_store for pre-chunked content.
