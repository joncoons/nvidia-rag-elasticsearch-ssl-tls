...
"""
Direct embedding and ES write path — bypasses nv-ingest entirely.

Designed for nemoretriever-parse pre-chunked text files: each file already
contains one text chunk; we embed it and bulk-write to ES asynchronously.

Also exposes ``ingest_text`` for HTML / plain-text sources that need
client-side character-level chunking before embedding.
...
async def ingest_text(
    ...
    logger.info(
        "direct_ingest.ingest_text: %d/%d chunks written — %s",
...
async def ingest_chunk_files(
    ...
    logger.info(
        "direct_ingest.ingest_chunk_files: %d/%d chunks written to collection=%s",
